...
#setwd(curdir)
  ...
  if(as.numeric(cores)<3){
    ...
    clusterExport(c1=c1,varlist=c("xcmsScriptPath","parsefiles","Fname","varstemp","vars",
                                  "hit1","hit2","xtemp","ytemp","xtempv","ytempv","xyCoord",
                                  "hit","totalfiles","dtafiles"))
    ...
    counter<-0
    invisible(clusterApplyLB(c1,parsefiles,function(name){
      counter<<-counter+1
      currentFile=name
      ...
      Fname[counter]=parsefiles[counter]
      close(par1)
      close(dta1)
    }))
    #todo
